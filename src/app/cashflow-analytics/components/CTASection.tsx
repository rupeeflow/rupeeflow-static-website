  ...
  return (
    <section className="relative py-20 md:py-28 bg-gradient-to-br from-[#020506] via-gray-900 to-[#020506] text-[var(--foreground)] overflow-hidden">
      ...
      <div className="container mx-auto px-6 md:px-8 lg:px-12 relative z-10">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/20 backdrop-blur-sm rounded-full mb-6">
            <Activity className="w-4 h-4 text-emerald-400" />
            <span className="text-sm font-medium text-emerald-400">Never Run Out of Cash</span>
          </div>
          <h2 className="fontheading text-4xl md:text-5xl lg:text-6xl font-bold mb-6">
            Master Your{' '}
            <span className="bg-gradient-to-r from-[#53BEC2] to-[#00EF64] bg-clip-text text-transparent">Cashflow</span>
          </h2>
          <p className="fontbody2 text-xl text-gray-300 mb-8 max-w-2xl mx-auto">Forecast with 94% accuracy and plan for liquidity needs</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            ...
            <Link href="/demo" className="px-8 py-4 border-2 border-emerald-500/50 text-[var(--foreground)] font-semibold rounded-xl hover:bg-emerald-500/10 hover:border-emerald-500 transition-all duration-200">See Demo</Link>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm text-gray-400">
            {['Free trial', 'No credit card', 'Setup in minutes'].map((feature, index) => (
              <div key={index} className="flex items-center gap-2"><Check className="w-4 h-4 text-emerald-400" /><span>{feature}</span></div>
            ))}
          </div>
        </motion.div>
      </div>
