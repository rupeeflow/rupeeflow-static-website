'use client';

import { motion } from 'framer-motion';
import { Zap } from 'lucide-react';

interface MockupProps {
  className?: string;
}

/**
 * BillPaymentMockup - Animated mockup showing bill payment interface
 * 
 * Features:
 * - Floating card with bill payment interface
 * - Amount display, biller name, and payment button
 * - Floating animation with y-axis movement between -7px and 0px
 * - 5 second loop duration for smooth, ambient animation
 * 
 * Requirements: 4.1, 4.3, 4.4, 11.7
 */
export default function BillPaymentMockup({ className = '' }: MockupProps) {
  return (
    <motion.div
      animate={{
        y: [0, -7, 0],
      }}
      transition={{
        duration: 5,
        repeat: Infinity,
        ease: 'easeInOut',
      }}
      className={`relative ${className}`}
    >
      {/* Glow effect behind mockup */}
      <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 blur-2xl rounded-3xl" />
      
      {/* Bill payment card */}
      <div className="relative bg-[var(--card)] rounded-2xl shadow-xl border border-gray-200 p-6 max-w-sm mx-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center">
              <Zap className="w-5 h-5 text-[var(--foreground)]" />
            </div>
            <div>
              <p className="text-xs text-gray-500">Electricity Bill</p>
              <p className="text-sm font-semibold text-gray-900">State Power Co.</p>
            </div>
          </div>
          <div className="px-2 py-1 bg-emerald-50 rounded-full">
            <span className="text-xs font-medium text-emerald-700">Due Soon</span>
          </div>
        </div>

        {/* Bill details */}
        <div className="bg-gray-50 rounded-xl p-4 mb-6">
          <div className="flex items-baseline justify-between mb-3">
            <span className="text-sm text-gray-600">Amount Due</span>
            <div className="text-right">
              <span className="text-2xl font-bold text-gray-900">₹2,450</span>
              <span className="text-sm text-gray-500">.00</span>
            </div>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">Due Date</span>
            <span className="font-medium text-gray-900">Dec 28, 2024</span>
          </div>
        </div>

        {/* Bill information */}
        <div className="space-y-2 mb-6">
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">Consumer ID</span>
            <span className="font-mono text-gray-900">1234567890</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">Billing Period</span>
            <span className="text-gray-900">Nov 2024</span>
          </div>
        </div>

        {/* Payment button */}
        <button className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-[var(--foreground)] font-semibold py-3 px-4 rounded-xl hover:from-emerald-600 hover:to-teal-700 transition-all duration-200 shadow-lg shadow-emerald-500/25">
          Pay Now
        </button>

        {/* Footer note */}
        <p className="text-xs text-center text-gray-500 mt-4">
          Instant payment confirmation
        </p>
      </div>
    </motion.div>
  );
}
