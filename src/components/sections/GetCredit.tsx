...
export default function GetCredit() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative bg-[var(--background)] pt-10 md:pt-15 pb-16 md:pb-24 overflow-hidden"
    >
      <Container className="relative">
        ...
        <div className="relative h-[90px] sm:h-[120px] md:h-[200px] mb-8 sm:mb-12 md:mb-16 px-4 md:px-0">
          ...
          <div className="gc-pill absolute left-0 top-[40px] sm:top-[60px] md:top-40 w-[260px] sm:w-[320px] h-[46px] sm:h-[60px] z-10">
            ...
            <span className="absolute left-5 sm:left-8 top-1/2 -translate-y-1/2 text-black text-[14px] sm:text-[20px] font-semibold">
              GET CREDIT
            </span>
          </div>
        ...
        <div className="gc-cards grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-x-10 md:gap-y-12 relative z-50 px-4 md:px-0">
          {creditCards.map((card, i) => (
            <div
              key={i}
              className="gc-card group relative min-h-[200px] md:h-[240px] flex flex-col rounded-2xl p-5 md:p-6 overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:-translate-y-1 bg-[var(--card)] border border-[var(--border)]"
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <h3 className="text-black text-lg md:text-xl font-semibold pr-12 transition-colors duration-300 ease-in-out group-hover:text-[var(--primary)]">
                  {card.title}
                </h3>
                ...
                <div className="mt-auto pt-4">
                  <Button href={card.href} className="group inline-flex rounded-full px-5 md:px-6 py-2 text-xs md:text-sm font-medium items-center gap-2">
                    <span className="flex items-center gap-2">
                      <span>Apply now</span>
                      <svg className="w-3 h-3 transition-transform duration-200 group-hover:translate-x-1" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
